...
void GC_getDirAndType(){
	...
	if(dirV != -1){
		PilaO.push(dirV);
		if(				  dirV<2000  || (dirV>=8000  && dirV<10000)){ PilaO.push(dirV); PilaT.push(1); } else
		if((dirV>=2000 && dirV<4000) || (dirV>=8000  && dirV<10000)){ PilaO.push(dirV); PilaT.push(2); } else
		if((dirV>=4000 && dirV<6000) || (dirV>=10000 && dirV<12000)){ PilaO.push(dirV); PilaT.push(3); } else
		if((dirV>=6000 && dirV<8000) || (dirV>=12000 && dirV<14000)){ PilaO.push(dirV); PilaT.push(4); } else 
	...
	if(!PilaO.empty()){
		if(Poper.top()==1 || Poper.top() == 2){
			...
			if(tipoR!=0){
				...
				addCuad(operador,dir2,dir1,dirT);
				PilaO.push(dirT);
				PilaT.push(tipoR);
			}else{
